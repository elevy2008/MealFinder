...
function App() {
  ...
  // Default selected types for "I want to help" mode
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Use different location data based on mode
          const locationsData = mode === 'need-help' 
            ? remainingLocations
            : helpLocations;
          
          const sorted = sortLocationsByDistance(locationsData, newUserLocation);
        ...
        (error) => {
          ...
          const sorted = sortLocationsByDistance(locationsData, defaultLocation);
          setSortedLocations(sorted);
          setIsLoading(false);
        },
        {
...
<MenuContent isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} />
      ...
      <div style={{ position: 'fixed', right: '0px', top: '15px', width: '300px', zIndex: 1001 }}>
        <AccordionList 
          locations={sortedLocations.filter(loc => 
            mode === 'need-help' 
              ? selectedTypes.includes(loc.type) 
              : helpSelectedTypes.includes(loc.type)
          )} 
          isLoading={isLoading} 
      ...
      <div style={{ position: 'relative', height: '100vh', zIndex: 1 }}>
        <MapContainer center={center} zoom={13} style={{ height: '100%', width: '100%' }}>
          ...
          {sortedLocations
            ...
            .map((location, index) => {
              // Determine which icon to use based on mode and location type
              let markerIcon;
              let typeLabel;
              
              if (mode === 'need-help') {
                // Icons for "I need help" mode
                markerIcon = location.type === 'food_truck' ? DefaultIcon : 
                             location.type === 'pantry' ? PantryIcon : ShelterIcon;
                             
                typeLabel = location.type === 'food_truck' ? 'Food Truck' : 
                            location.type === 'pantry' ? 'Food Pantry' : 'Shelter';
              } else {
                // Icons for "I want to help" mode
                switch(location.type) {
                  case 'volunteer':
                    markerIcon = VolunteerIcon;
                    typeLabel = 'Volunteer Opportunity';
                    break;
                  case 'donation':
                    markerIcon = DonationIcon;
                    typeLabel = 'Donation Center';
                    break;
                  case 'advocacy':
                    markerIcon = AdvocacyIcon;
                    typeLabel = 'Advocacy Organization';
                    break;
                  case 'support':
                    markerIcon = SupportIcon;
                    typeLabel = 'Support Service';
                    break;
                  case 'community':
                    markerIcon = CommunityIcon;
                  ...
                  <Popup>
                    <div>
                      ...
                      <a href={location.link} target="_blank" rel="noopener noreferrer">
                        {mode === 'need-help' ? 'View on Google Maps' : 'Learn More'}
                      </a>
                    </div>
                  </Popup>
                </Marker>
